...
export default function ChatWindow({ token, selectedUser, setMessagePreviews, setContacts}) {
      ...
      const socketClient = connectSocket(
        ...
        (msg)=>{
          ...
          if(msg.receiverId === currentUser && msg.id){
            ...
            fetch(`https://e2ee-chat.duckdns.org/api/api/messages/delivered/${msg.id}`,{
              method:"POST",
  ...
  useEffect(()=>{
    ...
    fetch(`https://e2ee-chat.duckdns.org/api/api/messages/${conversationId}`,{
  ...
  useEffect(()=>{
    ...
    messages
      ...
      .forEach(m=>{
        ...
        fetch(`https://e2ee-chat.duckdns.org/api/api/messages/read/${m.id}`,{
          method:"POST",
          headers:{Authorization:"Bearer "+token}
        });
  ...
  useEffect(()=>{
    ...
    fetch(`https://e2ee-chat.duckdns.org/api/api/profile/${selectedUser}`,{
      headers:{Authorization:"Bearer "+token}
    })
    .then(res=>res.json())
    .then(setUserProfile);
